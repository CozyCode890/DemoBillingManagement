...
try {
    Write-Host "Dang thuc thi 01_schema.sql..." -ForegroundColor DarkGray
    Get-Content '.\sql\01_schema.sql' -Raw -Encoding UTF8 |
        & $mysql -u $user -h 127.0.0.1 -P 3306 --default-character-set=utf8mb4
    if ($LASTEXITCODE -ne 0) { throw "Loi khi thuc thi 01_schema.sql" }

    Write-Host "Dang thuc thi 04_routines.sql..." -ForegroundColor DarkGray
    Get-Content '.\sql\04_routines.sql' -Raw -Encoding UTF8 |
        & $mysql -u $user -h 127.0.0.1 -P 3306 --default-character-set=utf8mb4
    if ($LASTEXITCODE -ne 0) { throw "Loi khi thuc thi 04_routines.sql" }
